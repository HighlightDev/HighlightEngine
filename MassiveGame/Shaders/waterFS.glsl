...
const float materialReflectivity = 1.1f;
const float materialShineDamper = 100.0f;
...
vec4 specularPointLight(vec3 specularNormal, vec3 ligthDiffuseV, vec3 InAttenuation, vec3 specularColor, float softEdge)
{
	...
	//Specular light calculations
		...
		if (max(dot(specularNormal,  normDiffVec), 0.0) > 0.0)	//Если источник света освещает поверхность
		{	
			vec3 unitCameraV = normalize(eyeCameraVec);		
