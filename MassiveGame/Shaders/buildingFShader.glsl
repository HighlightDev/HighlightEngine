...
vec3 phongModelDirectLight(vec3 diffuseNormal, vec3 specularNormal, vec3 toLightVector, vec3 specularMaterial)
{
	...
	{
		if (specularMaterial.r > 0.01 || specularMaterial.g > 0.01 || specularMaterial.b > 0.01 ) //Если материал отражения сводит его на нет - не вычисляем отражения
		{
		...
		vec3 diffuseColor, vec3 specularColor, vec3 specularMaterial)
{
	...
	//Specular light calculations
		...
		if (brightness > 0.0)	//Если источник света освещает поверхность
		{	
