...
{ 
	...
	mat4 lightModelViewMatrix = viewMatrix * worldMatrix;
	surfaceDiffuseNormal = (worldMatrix * vec4(Normals, 0.0)).xyz;
	...
	vec3 tangDiffuse = normalize((worldMatrix * vec4(Tangent, 0.0)).xyz); 
	vec3 bitangDiffuse = normalize((worldMatrix * vec4(Bitangent, 0.0)).xyz);
